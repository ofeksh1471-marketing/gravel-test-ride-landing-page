...
exports.handler = async (event) => {
  ...
  if (debugMode === '2') {
    const testPayload = {
      ...
      contacts: [{
        ...
        externalName: 'RecyclesGravelTestRide'
      }]
    };
  ...
  try {
    ...
    if (data.marketingApproved !== true) {
      return {
        statusCode: 200,
        body: JSON.stringify({ ok: true, skipped: true, reason: 'Marketing approval was not checked' })
      };
    }
    ...
    const activeTrailPayload = {
      group: groupId,
      contacts: [{ contact, externalId, externalName: 'RecyclesGravelTestRide' }]
    };
